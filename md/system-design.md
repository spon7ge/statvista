...
| `/mlb/prop_picks` | DFS **player board**: PrizePicks / Underdog tabs (format/legs defaults hidden — power/4 vs standard/4); one card per player with **View X props** CTA; sort by unique-stat count desc; Team multi-select + player name search | `useMlbProps` | `GET /api/mlb/props/today?app=&format=&legs=` | Client `groupMlbPropPlayers` aggregates rows; PrizePicks seed from latest Supabase snapshot only (`fetch_latest_prizepicks("mlb")`, no Parlay PP fallback; empty → `prizepicks_unavailable`); Underdog from Supabase snapshot; each row carries `books_main` (per-book main O/U) for detail grid; Parlay sportsbooks (DraftKings, FanDuel, BetMGM, Caesars, Kalshi, Fliff, bet365) from latest `odds.mlb_parlay_api_odds` snapshot only (`fetch_latest_parlay_api_odds("mlb")` — no live Parlay index fallback; empty snapshot → NL Parlay columns + `parlay_unavailable`); live Parlay fetch is throttled write-only side-effect (`maybe_persist_parlay_props(..., league="mlb")`); ProphetX/Novig/Pinnacle from Supabase scrapers; paginates 20 **players** per page |
| `/mlb/prop_picks/player/:playerSlug?app=` | Per-player main-line odds grid (BettingPros-style) | `useMlbProps` | same `GET /api/mlb/props/today?app=&format=&legs=` | `findPlayerBySlug` + `uniqueStatRows`; `MlbPlayerPropsOddsGrid` reads `books_main` (ProphetX, Novig, DraftKings, FanDuel, BetMGM, Caesars, Kalshi, Fliff, bet365, Pinnacle — main lines only, NL when missing; no OPEN/BEST); Parlay books from `odds.mlb_parlay_api_odds` snapshot (same serve rule as board); unknown slug → empty state + link back to board |
...
MlbPropPicksPage
  ...
  → mlb.props.get_mlb_props_today()
       ├─ seed: Supabase PrizePicks / Underdog only (no Parlay PP fallback)
       ├─ Parlay sportsbooks: fetch_latest_parlay_api_odds("mlb") → odds.mlb_parlay_api_odds
       │     (live Parlay fetch → maybe_persist_parlay_props(..., league="mlb") throttled write only)
       ├─ exact-line indexes: PX/Novig/Pinnacle from Supabase scrapers
       └─ prop_fair + prop_formats + books_main (empty Parlay snapshot → parlay_unavailable, NL Parlay cols)
  → client groupMlbPropPlayers + filterMlbPropPlayers (page size 20)
```
