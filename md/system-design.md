...
| `/wnba/standings` | East / West standings | `useWnbaStandings` | `GET /api/wnba/standings` | ESPN via `app.core.outbound_cache` (memory + `data/cache/outbound`, TTL + SWR) |
...
- WNBA ESPN standings and roster fetches use shared `app.core.outbound_cache` (memory + disk under `data/cache/outbound`, per-key TTL, stale-while-revalidate, coalescing, host rate limit/backoff). Source remains ESPN.
