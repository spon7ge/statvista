...
| `/mlb/prop_picks` | Filterable DFS +EV ranked board (hybrid rows + expand) | `useMlbProps` | `GET /api/mlb/props/today?app=&format=&legs=` | DFS-first from Supabase MLB PrizePicks/Underdog snapshots; server fair/edge/tier/recency (Tier 1 PX+Novig → Tier 2 DK+FD → Tier 3 Soft Consensus equal avg of exact-line soft books incl. Pinnacle/Caesars/Kalshi/bet365/BetMGM/Fanatics/Hard Rock/Fliff when Tier 1/2 empty; expand still shows soft books as `(cmp)`); defaults PrizePicks power 4 legs; client filters Stat/Team/Side via `filterMlbPropPicks` |
