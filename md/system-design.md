...
| `/mlb/prop_picks` | Sportsbook **research table**: one row per player + market + exact line + side; composite cell (headshot / **name · matchup** / market below); Line, Odds chips (logo + American only when that book posts a price; PrizePicks uses -137); omit rows with empty Odds; IP, L5/L10/L15, H2H (this season + last season vs opponent, pooled); **Game** (day's slate, away @ home) + Team + **Bookmaker** + **Proposition** (market) + **Over/Under** + **Hit rate** (L5/L10/L15 highest→lowest) + player name search; no PrizePicks/Underdog tabs | `useMlbPropBoard` | `GET /api/mlb/props/board` | Client `filterMlbPropBoardRows` (game + team + book + market + side + name; selected books trim Odds chips; drop rows with no posted Odds); default sort game start / name / stat / Over then Under / line; hit-rate control sorts L5/L10/L15 desc; **30 rows per page** with Previous/Next; empty copy “No board yet” |
...
MlbPropPicksPage
  ...
  → mlb.prop_board.get_mlb_prop_board()
       ...
       ├─ emit Over + Under rows; IP from PX → Novig → Pinnacle → DK two-way
...
| GET | `/api/mlb/props/board` | `mlb.prop_board` (sportsbook + DFS mains clustered by exact line; IP / ranks / L5–L15; 200 even when enrichments fail) |
