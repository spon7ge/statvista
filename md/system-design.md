...
| `/games/:espnEventId` | Game detail: scheduled → MLB-parity pregame (broadcast header with record + last 10; centered Preview / Away / Home / **Props**); Preview two-column (left: Projected Starters → Game Info → Matchup Prediction → Game Leaders PPG/RPG/APG; right: Odds board → Team Stats + `#rank` → Injuries); Away/Home → team preview (PPG/RPG/APG leaders + roster averages); Props → PrizePicks / Underdog sub-tabs filtering today’s props to game teams; live/halftime or final → broadcast header + Summary \| Box (Scoring plays \| All plays; Summary right rail: linescore, team stats, win probability, shot chart, Game Info; Box: stacked away/home box score) | `useGameDetail`, `useWnbaOdds`, `useWnbaProps`, `useWnbaTeamPreview(espnEventId, side)` | `GET /api/wnba/games/{id}`, `GET /api/wnba/odds/today` (Preview), `GET /api/wnba/props/today` (Props tab; client-filter), `GET /api/wnba/games/{id}/team-preview?side=` (Away/Home) | ESPN summary + standings/team stats/roster for enrichment; RotoWire starters; soft-fail empty sections; no Player of the Game or pitch zone |
