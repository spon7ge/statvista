...
export type MlbPropBoardSortKey =
  | "player"
  | "line"
  | "odds"
  | "ip"
...
function firstOddsAmerican(row: ApiMlbPropBoardRow): number | null {
  const quote = orderedBoardBooks(row.books).find((book) => book.american != null);
  return quote?.american ?? null;
}

export function compareMlbPropBoardDefault(
  a: ApiMlbPropBoardRow,
  b: ApiMlbPropBoardRow,
): number {
...
export function sortMlbPropBoardRows(
  rows: ApiMlbPropBoardRow[],
  sort: MlbPropBoardSort | null,
): ApiMlbPropBoardRow[] {
  return [...rows].sort((a, b) => {
    ...
    return compareMlbPropBoardDefault(a, b);
  });
}
