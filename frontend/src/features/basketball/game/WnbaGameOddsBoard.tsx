...
import {
  formatAmericanOdds,
  type WnbaOddsBoardTile,
  type WnbaOddsBookBoardView,
} from "../lib/wnbaOddsBoard";
import type { GameDetail, GameDetailTeam } from "../lib/types";

type Props = {
  detail: Pick<GameDetail, "away" | "home">;
  boards: WnbaOddsBookBoardView[];
  isPending?: boolean;
};
...
function formatTileLine(tile: WnbaOddsBoardTile): string | null {
...
function OddsTile({ tile }: { tile: WnbaOddsBoardTile }) {
...
function TeamOddsRow({
  ...
  team: GameDetailTeam;
  row: WnbaOddsBookBoardView["rows"][number];
}) {
  return (
...
export function WnbaGameOddsBoard({ detail, boards, isPending }: Props) {
