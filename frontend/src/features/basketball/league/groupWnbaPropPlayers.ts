...
export type WnbaPropPlayerCard = {
  player_name: string;
  player_slug: string;
  prop_count: number;
...
function playerGroupKey(row: ApiWnbaPropRow): string | null {
...
function assignUniqueSlugs(cards: WnbaPropPlayerCard[]): void {
...
export function groupWnbaPropPlayers(props: ApiWnbaPropRow[]): WnbaPropPlayerCard[] {
  const byPlayer = new Map<string, ApiWnbaPropRow[]>();
  ...
  const cards: WnbaPropPlayerCard[] = [];
...
export function findPlayerBySlug(
  ...
  slug: string,
): WnbaPropPlayerCard | null {
  return players.find((p) => p.player_slug === slug) ?? null;
}

export function uniqueStatRows(rows: ApiWnbaPropRow[]): ApiWnbaPropRow[] {
  const seen = new Set<string>();
  const out: ApiWnbaPropRow[] = [];
