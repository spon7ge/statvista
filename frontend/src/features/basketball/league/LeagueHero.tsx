...
export function LeagueHero({ league, dateEt }: LeagueHeroProps) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 pt-12 sm:px-6 sm:pt-16">
      <div className="grid items-center gap-8 border-b border-line pb-10 sm:grid-cols-[1.2fr_0.8fr] sm:gap-12 sm:pb-12">
        <div className="max-w-2xl">
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <span className="kicker">{content.label}</span>
            <time dateTime={dateEt ?? undefined} className="kicker">
              {dateLabel}
            </time>
          </div>
          <h1 className="page-title">{content.title}</h1>
          <p className="mt-3 max-w-xl text-[16px]">{content.blurb}</p>
        </div>
