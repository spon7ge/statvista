...
import {
  orderedWnbaBoardBooks,
  orderedWnbaDfsBooks,
  sortWnbaPropBoardRows,
  type WnbaPropBoardSort,
  type WnbaPropBoardSortKey,
} from "./sortWnbaPropBoard";
const VISIBLE_ODDS_CHIPS = 3;
export const WNBA_PROP_BOARD_PAGE_SIZE = 30;
...
const HIT_SORT_KEYS = new Set<WnbaPropBoardSortKey>(["l5", "l10", "l15", "h2h"]);
...
function formatMatchup(row: ApiWnbaPropBoardRow): string | null {
...
function BookChip({
  ...
  const inner = (
    <>
      ...
      <span className="text-[12px] text-c3">
...
function OddsOverflow({ chips }: { chips: ApiWnbaPropBoardRow["books"] }) {
...
function DfsCell({ row }: { row: ApiWnbaPropBoardRow }) {
  const chips = orderedWnbaDfsBooks(row.dfs ?? []).filter(
    (chip) => postedAmerican(chip.book, chip.american) != null,
  );
  if (chips.length === 0) {
    return (
      <div data-testid="dfs-cell" className="text-sm text-c3">
        —
      </div>
    );
  }
...
function OddsCell({ row }: { row: ApiWnbaPropBoardRow }) {
  const books = orderedWnbaBoardBooks(row.books).filter(
    (chip) => postedAmerican(chip.book, chip.american) != null,
  );
...
function PlayerIcon({ row }: { row: ApiWnbaPropBoardRow }) {
  const initial = (row.player_name.trim()[0] ?? "?").toUpperCase();
  const teamLogo = row.team_abbrev ? wnbaTeamLogoUrl(row.team_abbrev) : null;
  return (
    <div
...
function CompositeCell({ row }: { row: ApiWnbaPropBoardRow }) {
...
export function WnbaPropPicksTable({
  rows,
  isLoading = false,
  isError = false,
  hitRateWindow = null,
}: WnbaPropPicksTableProps) {
  const [sort, setSort] = useState<WnbaPropBoardSort | null>(null);
  const [page, setPage] = useState(0);
  const sorted = useMemo(() => sortWnbaPropBoardRows(rows, sort), [rows, sort]);
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(sorted.length / WNBA_PROP_BOARD_PAGE_SIZE),
  );
  const safePage = Math.min(page, totalPages - 1);
  const start = safePage * WNBA_PROP_BOARD_PAGE_SIZE;
  const pageRows = sorted.slice(start, start + WNBA_PROP_BOARD_PAGE_SIZE);
  const end = start + pageRows.length;
  const showPager = sorted.length > WNBA_PROP_BOARD_PAGE_SIZE;

  function onSort(key: WnbaPropBoardSortKey) {
    setPage(0);
    setSort((current) => {
  ...
  return (
    <section className="space-y-3">
      {isLoading ? (
        <div
          aria-label="Loading WNBA prop picks"
        ...
        <>
          ...
          <table className="w-full min-w-[64rem] border-separate border-spacing-x-0 border-spacing-y-1.5 text-left">
            <thead className="sticky top-0">
              <tr className="border-b border-line text-[12px] font-semibold uppercase tracking-wide text-c3">
                {COLUMNS.map((column) => {
                  const isHit =
                    column.sortable &&
                    HIT_SORT_KEYS.has(column.key as WnbaPropBoardSortKey);
                  if (!column.sortable) {
                    return (
                      <th
                        key={column.key}
                  ...
                  return (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() =>
                          onSort(column.key as WnbaPropBoardSortKey)
                        }
            ...
            <tbody>
              {pageRows.map((row) => (
                <tr
                  key={`${row.player_name}:${row.stat}:${row.side}:${row.line}:${row.game_id ?? ""}`}
                ...
                >
                  ...
                  <td
                    className={`px-2 py-2 text-sm text-c3 ${ROW_BOX_MIDDLE}`}
