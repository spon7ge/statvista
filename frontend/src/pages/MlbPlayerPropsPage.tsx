...
import {
  ...
  groupMlbPropPlayers,
  uniqueStatRows,
  type MlbPropPlayerCard,
} from "@/features/mlb/league/groupMlbPropPlayers";
...
function PlayerDetailHeader({ player }: { player: MlbPropPlayerCard }) {
  ...
  return (
    <header className="flex items-center gap-4">
      {showImg ? (
        ...
        <span
          data-testid="mlb-player-props-headshot-fallback"
...
export function MlbPlayerPropsPage() {
  const { playerSlug = "" } = useParams<{ playerSlug: string }>();
  const [params] = useSearchParams();
  const app = appFromSearch(params.get("app"));
  const format = formatForApp(app);
  const boardHref = `/mlb/prop_picks?app=${app}`;

  const { data, isLoading, isError, isFetched } = useMlbProps({
    app,
    format,
    legs: BOARD_LEGS,
  });
  ...
  const players = useMemo(
    () => groupMlbPropPlayers(data?.props ?? []),
    [data],
  );
  ...
  if (showLoading) {
    body = (
      <div
        ...
        aria-label="Loading MLB player props"
      >
    ...
    body = (
      <div className="mx-auto max-w-6xl space-y-6 px-4 pb-16 sm:px-6 sm:pb-20">
        ...
        <MlbPlayerPropsOddsGrid markets={markets} />
      </div>
    );
  }
