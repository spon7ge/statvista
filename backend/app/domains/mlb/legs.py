"""Assemble GET /api/mlb/legs: DFS seed → exact-line two-ways → pricer."""
...
from app.domains.mlb.schemas_scoreboard import MlbGame
...
def clear_mlb_legs_cache() -> None:
    _cache.clear()


def _empty_rejected_summary() -> MlbLegsRejectedSummary:
    return MlbLegsRejectedSummary(
...
def _team_game_index(games: list[MlbGame]) -> dict[str, MlbGame]:
    by_team: dict[str, MlbGame] = {}
    for game in games:
...
async def get_mlb_legs(*, app: str, format: str, legs: int) -> MlbLegsResponse:
    ...
    if app == "prizepicks":
        dfs_rows = odds_snapshots.fetch_latest_prizepicks("mlb")
        empty_warning = "prizepicks_unavailable"
    else:
        dfs_rows = odds_snapshots.fetch_latest_underdog("mlb")
    ...
    px_idx = _index_with_stake(
        odds_snapshots.fetch_latest_prophetx("mlb", mains_only=False),
        player_field="player_name",
        stat_field="stat_name",
    )
    novig_idx = _index_with_stake(
        odds_snapshots.fetch_latest_novig("mlb", mains_only=False),
        player_field="player_name",
        stat_field="stat_name",
    )
    pin_idx = _index_with_stake(
        odds_snapshots.fetch_latest_pinnacle("mlb"),
        player_field="player_name",
        stat_field="market_type",
    )
    parlay_rows = odds_snapshots.fetch_latest_parlay_api_odds("mlb")
    if not parlay_rows:
    ...
    try:
        ...
        logger.warning("MLB legs scoreboard unavailable: %s", exc)
        team_games = {}

    try:
        roster = await mlb_roster.get_mlb_player_index()
    except Exception as exc:
        logger.warning("MLB legs roster unavailable: %s", exc)
        roster = {}
    ...
    for (player_key, stat_key, line_f), bucket in seeded.items():
        ...
        if game is not None and game.status in _LOCKED:
            continue
        evaluated += 1
        game_id = game.mlb_game_pk if game is not None else None
        matchup = (
